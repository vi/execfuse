               ...
               off_t offset, struct fuse_file_info *fi)
{
	...
	for(;;) {
		...
		int	ret = chunked_buffer_read(r, buf_, EXECFUSE_CHUNKSIZE-1, offset_), l;
		struct stat	st;
...
static struct chunked_buffer* call_script_stdout_ret(const char* script_name, const char* param1, const char* param2, int* call_return_code)
{
	...
	ret = call_script_ll(script_name
		,params
		,NULL, NULL
		,&call_script_stdout_write, (void*)&cbuf
		);

	if (call_return_code != NULL)
		*call_return_code = ret;
...
int main(int argc, char *argv[])
{
	...
	ret = call_script_simple("check_args", SCRIPT_API_VERSION);
	if (ret && ret != ENOSYS)
		return ret;
